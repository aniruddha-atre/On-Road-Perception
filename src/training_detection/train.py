# src/training/train_wandb_explicit.py
import os
...
def parse_args():
    ...
    ap.add_argument("--cache", default="disk")   # 'disk' avoids RAM spikes on Windows
    return ap.parse_args()

def main():
    args = parse_args()

    # Keep the console clean and force W&B usage
    SETTINGS.update({"wandb": True})

    os.environ.setdefault("WANDB_MODE", "online")
    os.environ.setdefault("WANDB_PROJECT", args.project)

    # ---- 1) Explicit W&B init (associates Ultralytics logs with this run) ----
    run = wandb.init(
        ...
        settings=wandb.Settings(start_method="thread"),  # safe on Windows
    )
    ...
    # ---- 3) Locate best.pt and log to W&B as an Artifact ---------------------
    # Ultralytics places: runs/detect/<name>/weights/best.pt (task 'detect' for YOLO v8/v9)
    ...
    if best_path.exists():
        ...
        # also store the data/configs used for full reproducibility
    ...
    # Also log the final metrics dict returned by Ultralytics:
